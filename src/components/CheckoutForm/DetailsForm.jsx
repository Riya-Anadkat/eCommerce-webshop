...
const DetailsForm = ({ checkoutToken }) => {
    ...
    const fetchExtraFields = async(checkoutTokenId) => {
        const { extrafields } = await commerce.services.localeListShippingCountries(checkoutTokenId);
        
        console.log(extrafields);
        setExtraFields(extrafields);
    }
    useEffect(() =>{
        fetchExtraFields(checkoutToken.id);
    }, []);
  ...
  const Products = ({ product }) => {
    return (
      <div>
      <Typography variant="subtitle1" gutterBottom align="center">Details for {product.name}</Typography>
...
//<FormInput required name={product.extrafields.name} label={product.extrafields.name} />
    return (
        <>
...
//                     <Grid item key = {checkoutToken.id} xs={12} sm={6} lg={3}>
//                         <ExtraField extrafield={ extrafield } />
//                     </Grid>
